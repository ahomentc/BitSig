...
exports.updateNumberAddressSignedWhenSign = functions.database.ref('/tokens/{token_id}/signer_users/{uid}/num_signer').onCreate((snapshot, context) => {
	const token_id = context.params.token_id;
	...
	return snapshot.ref.root.child('/numSignersForToken/' + token_id).transaction(counter_value => {
    	...
    	return snapshot.ref.root.child('/numSignersForToken/' + token_id).once('value', num_signers => {
    		var val = parseInt(num_signers.val())
			promises.push(snapshot.ref.root.child('/tokens/' + token_id + '/signer_users/' + uid + '/num_signer').set(val));
			promises.push(snapshot.ref.root.child('/users/' + uid + '/tokens_signed/' + token_id).set(val));
			return Promise.all(promises);
		}).catch(() => {return null});
	}).catch(() => {return null});
})
